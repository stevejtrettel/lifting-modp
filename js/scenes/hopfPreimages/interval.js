...
let curve = function(t){
    t = t/(2*Math.PI);
    return {theta: t,phi: Math.PI/2 +0.1*Math.sin(4*t)};
}
let hopf = new HopfPreimage(curve);

base.add(hopf.getBase(0xa1d6f7));
base.add(hopf.getBaseCurve(colors.yellow));
fibers.add(hopf.getPreimageCurve(colors.yellow,false));

base.add(hopf.getBasePointOnCurve(2.*Math.PI,colors.red));
fibers.add(hopf.getPreimagePointOnCurve(2.*Math.PI,colors.red));

base.add(hopf.getBasePointOnCurve(0,colors.red));
fibers.add(hopf.getPreimagePointOnCurve(0,colors.red));
