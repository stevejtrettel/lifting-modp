...
let curve = function(t){
    return {theta: t,phi: 0.5+0.2*Math.sin(2*t)*Math.sin(2*t)};
}
let hopf = new HopfPreimage(curve);

base.add(hopf.getBase(0xa1d6f7));
base.add(hopf.getBaseCurve(colors.purple));
fibers.add(hopf.getPreimageCurve(colors.purple,false));

// base.add(hopf.getBasePointOnCurve(0,colors.blue,0.05));
// fibers.add(hopf.getPreimagePointOnCurve(0,colors.blue,0.01));
//



 //base.rotateX(-0.5);
fibers.position.set(0.,1.5,1);
