...
const tau = new Vector2(0,Math.sqrt(5));

//these *dont* come directly from tau, as that's the wrong representation of the rectangle
...
const curveArea = 2*Math.PI/3;
const curveLength = 2*Math.PI/3*Math.sqrt(5);
...
//takes in points in the standard FD gen by 1 and tau = sqrt(5)i
...
let fromTauCoords = function(data) {
    ...
    let sin = Math.sin(Math.atan(Math.sqrt(5)));
    let cos = Math.cos(Math.atan(Math.sqrt(5)));
    let u = cos*x - sin*y;
    let v = sin*x + cos*y;
